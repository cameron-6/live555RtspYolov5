    ...
    for (int i = 0; i < grid_h; i++) {
      for (int j = 0; j < grid_w; j++) {
        ...
        if (box_confidence >= thres_i8) {
          ...
          //对80个类别的预测结果做个排序, 拿到这个预测框所有80类别中, 它的置信度最高的那个结果
                 ...
                 std::vector<float>& qnt_scales, detect_result_group_t* group)
{
  ...
  int validCount0 = 0;//非常重要, 就是在80x80这个维度, 所有超过置信度阈值的预测框的总个数.

  //anchor代表锚框，数组中相邻两个值表示框的宽高
  //const int anchor0[6] = {10, 13, 16, 30, 33, 23}; 当跨度为8的时候, 这3个锚框(两个值分别代表宽高)分别去框定相对方形, 瘦高型, 矮胖型的物体,
  ...
  int validCount1 = 0; //非常重要, 就是在80x80这个维度, 所有超过置信度阈值的预测框的总个数.
  ...
  int validCount2 = 0;//非常重要, 就是在80x80这个维度, 所有超过置信度阈值的预测框的总个数.
  ...
  //nms运算，可能在gird_cell的几个不同的anchor的置信度都过了阈值, 那么就看看他们跟置信度最高的那个box之间的交并比(IOU)关系如何. 这个值越高说明, 这两个区域差不多是同一个区域.
