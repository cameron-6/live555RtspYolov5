...
int RkEncoder::init()
{
	...
	// 帧间量化参数差值 (rc:qp_ip),值越小：I 帧和 P 帧画质接近，适合高流畅需求。
	...
	mpp_enc_cfg_set_s32(cfg, "rc:gop", 30); // 每秒 1 个关键帧(以fps=30为准)
	// 禁用 B 帧
	mpp_enc_cfg_set_s32(cfg, "rc:drop_b_frame", 1);
...
int RkEncoder::encode(void *inbuf, int insize, uint8_t *outbuf)
{
	...
	//LOG(NOTICE, "insize %d", insize);
	memcpy(mpp_buffer_get_ptr(m_buffer), inbuf, insize);
	//LOG(NOTICE, "memcpy success");
